...
export const authPlugin = new Elysia({ prefix: "/auth" })
    ...
    if (match) {
        ...
        }, config.accessTokenSecret, { expiresIn: "15m" });
        const refreshToken = jwt.sign({ userId: foundUser.userId }, config.refreshTokenSecret, { expiresIn: "1d" });
    ...
    try {
        ...
        const roles = Object.values(foundUser.roles || {}).filter((role) => typeof role === "number");
        const accessToken = jwt.sign({ userId: foundUser.userId, roles: roles }, config.accessTokenSecret, { expiresIn: "15m" });
        set.status = 200;
        return { status: "SUCCESS", body: { accessToken } };
    }
    catch (error) {
