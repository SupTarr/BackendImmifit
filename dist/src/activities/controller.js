...
import { CreateActivityBodySchema, ActivityIdParamsSchema, UserIdParamsSchema, EditActivityBodySchema, } from "./model.js";
...
export const activitiesPlugin = new Elysia({ prefix: "/activities" })
    ...
    .get("/:activityId", async ({ params, set }) => {
    ...
    .put("/:activityId", async ({ params, body, set, }) => {
    const { activityId } = params;
        ...
        if (img && img instanceof File) {
            if (existingActivity.img && existingActivity.img.id) {
                try {
                    ...
                    console.warn("Cloudinary: Failed to delete old image during update:", deleteError);
                }
            }
            ...
            console.warn("Received 'img' field in PUT request, but it was not a file.");
        }
        const finalUpdateData = { ...updateData };
        if (uploadedImgData) {
            finalUpdateData.img = uploadedImgData;
        }
        const updatedActivity = await Activities.findOneAndUpdate({ activityId }, { $set: finalUpdateData }, { new: true, runValidators: true });
        if (!updatedActivity) {
    ...
    .delete("/:activityId", async ({ params, set }) => {
    const { activityId } = params;
    try {
        const activity = await Activities.findOne({ activityId });
        if (!activity) {
        ...
        if (activity.img && activity.img.id) {
            try {
                ...
                console.error("Cloudinary delete error (activity deletion):", deleteError);
            }
        }
        const deleteResult = await Activities.deleteOne({ activityId });
        if (deleteResult.deletedCount === 0) {
            throw new NotFoundError("Activity not found during deletion attempt.");
        }
        set.status = 204;
        return;
    }
