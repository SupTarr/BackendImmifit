...
export const userPlugin = new Elysia({ prefix: "/users" })
    ...
    .put("/:userId/profile", async ({ params, body, set, }) => {
    const { userId } = params;
    ...
    try {
        ...
        if (height !== undefined && weight !== undefined && height > 0) {
            ...
            updatePayload.bmi = parseFloat((weight / (heightInMeters * heightInMeters)).toFixed(2));
        }
        else {
            if (profileData.hasOwnProperty("height") ||
        ...
        const updatedProfile = await Profile.findOneAndUpdate({ userId: userId }, updateOperation, {
            new: true,
            upsert: true,
            runValidators: true,
        ...
        set.status = 200;
        const { _id, userId: profileUserId, profileId, ...profileToReturn } = updatedProfile.toObject();
